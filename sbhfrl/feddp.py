...
class FedDPClient:
    """
    FedDP client: aligns representation space (prototypes) and parameter space (weights) to mitigate feature shift.
    """
    ...
    def __init__(self, client_id: int, loader: DataLoader, config: Dict):
        ...
        self.lambda_proto_align = float(config.get("lambda_proto_align", 0.2))
        self.lambda_param_align = float(config.get("lambda_param_align", 0.01))
        self.lambda_soft_label = float(config.get("lambda_soft_label", 0.1))
        self.lambda_contrast = float(config.get("lambda_proto_contrast", 0.1))
        self.proto_temperature = float(config.get("proto_temperature", 0.5))
    ...
    def _soft_labels(self, embeddings: torch.Tensor, global_proto: torch.Tensor) -> torch.Tensor:
        """Prototype-similarity-based soft labels."""
        sim = torch.matmul(F.normalize(embeddings, dim=1), global_proto.t()) / self.proto_temperature
        return F.softmax(sim, dim=1)

    def _param_alignment_loss(self, model: torch.nn.Module, global_state: Dict[str, torch.Tensor], device: torch.device) -> torch.Tensor:
        loss = torch.tensor(0.0, device=device)
        if self.lambda_param_align <= 0:
            return loss
        for name, param in model.named_parameters():
            ...
            loss = loss + F.mse_loss(param, ref)
        return loss
    ...
    def train(
        ...
        for _ in range(self.config.get("local_epochs", 1)):
            for images, labels in self.loader:
                ...
                if global_proto is not None:
                    ...
                    loss = loss + self.lambda_proto_align * F.mse_loss(F.normalize(embeddings, dim=1), proto_targets)
                    soft_target = self._soft_labels(embeddings, proto_device)
                    loss = loss + self.lambda_soft_label * _temperatured_kl(logits, soft_target, self.proto_temperature)

                if self.lambda_param_align > 0:
                    loss = loss + self.lambda_param_align * self._param_alignment_loss(model, global_state, device)
        ...
        proto_loader = DataLoader(
            self.loader.dataset,
            batch_size=self.config.get("proto_batch_size", 128),
            shuffle=False,
        )
        prototypes = compute_prototypes(model, proto_loader, self.config.get("num_classes", 10), device)
        return {
...
def run_feddp(config: Dict, device: torch.device) -> None:
    ...
    clients = [FedDPClient(idx, loader, config) for idx, loader in enumerate(loaders)]
